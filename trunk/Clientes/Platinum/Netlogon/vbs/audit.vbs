...
Set objFSO = CreateObject("Scripting.FileSystemObject")
  ...
  f.close
  execute constantes
set file = objFSO.GetFile(LOGUSER & "\Arquivos-" & computador & ".txt")		
If DateDiff("d", file.DateLastModified, Now) < 30 Then
objFSO.DeleteFile LOGUSER & "\Arquivos-" & computador & ".txt"
wscript.quit
End If 
...
set objTextFile = objFSO.CreateTextFile(LOGUSER & "\Arquivos-" & computador & ".txt", True)
